...
The native `mxcsr_probe` also printed `PASS` and then had a separate 104-byte LSan report with allocation stacks through `dlopen` and the dynamic loader, not through a solver allocation frame. It remains in the native LSan partition; a future CI failure there must be investigated, not hidden or called a solver pass.
