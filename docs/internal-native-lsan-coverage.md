...
The native `mxcsr_probe` also printed `PASS` and then had a separate 104-byte LSan report with allocation stacks through `dlopen` and the dynamic loader, not through a solver allocation frame. Run 36179216551 reproduced exactly this failure while the other seven native checks passed. The short-lived FP-mode probe now keeps both DSOs loaded until process exit, which is the load behavior it is designed to check; native solver lifecycle tests still independently run with LSan. The follow-up CI run determines whether this bounded probe correction resolves the loader-only report.
