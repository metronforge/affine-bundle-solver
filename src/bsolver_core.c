...
int main(int argc,char**argv){
 ...
 for(int ai=1;ai<argc;ai++){double*A=NULL;int m,n;if(!load_matrix(argv[ai],&A,&m,&n)){fprintf(stderr,"load fail %s\n",argv[ai]);continue;}double*x=malloc(n*sizeof(double)),*b=malloc(m*sizeof(double));uint64_t s=123;for(int j=0;j<n;j++)x[j]=gauss(&s);for(int i=0;i<m;i++)b[i]=dot(A+(size_t)i*n,x,n);run_case(argv[ai],A,b,x,m,n,1); stress_case("real-cons",A,b,x,m,n,100,4,2,2); if(m>n+2){b[m-1]+=0.15;run_case("real-incons",A,b,x,m,n,1);stress_case("real-X",A,b,x,m,n,100,4,2,2);}free(A);free(b);free(x);} {int m=4096,n=64;double*A=malloc((size_t)m*n*sizeof(double)),*b=malloc(m*sizeof(double)),*x=malloc(n*sizeof(double));gen_rare(A,b,x,m,n,0);stress_case("rare-s1",A,b,x,m,n,200,1,2,2);stress_case("rare-s4",A,b,x,m,n,200,4,2,2);b[m-1]+=0.2;stress_case("rare-X-s1",A,b,x,m,n,200,1,2,2);stress_case("rare-X-s4",A,b,x,m,n,200,4,2,2);free(A);free(b);free(x);} return 0;
}
