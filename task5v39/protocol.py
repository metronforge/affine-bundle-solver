"""Immutable v3.9 schedule and bootstrap-safe worker execution."""
...
from task5v39.runtime import bootstrap_environment
...
def run_dataset(schedule, *, runner, checkpoint, protocol):
    ...
    if checkpoint.exists():
        ...
        report = {
            "campaign_id": CAMPAIGN_ID, "protocol": protocol,
...
def worker_runner(*, arms, bundles, configuration, timeout_seconds=30):
    ...
    def run(item):
        ...
        command = [
            "taskset", "-c", config.cpu_set, sys.executable, "-m",
            "task5v36.worker", "--role", role, "--configuration", configuration,
            "--bundle", str(Path(bundles[item["slot_id"]]).resolve()),
...
def main():
    ...
    schedule = build_schedule(
        args.dataset, tuple(arms), tuple(args.slots),
        repetitions=args.repetitions, warmups=args.warmups, seed=args.seed,
    )
    protocol = {
        "campaign_id": CAMPAIGN_ID,
        "protocol_revision": "v3.9-bootstrap-safe-openblas",
        "dataset": args.dataset, "configuration": args.configuration,
        "configuration_policy": repr(CONFIGURATIONS[args.configuration]),
        "arms": arms,
        "bundles": {key: str(value.resolve()) for key, value in bundles.items()},
    ...
    report = run_dataset(
        ...
        runner=worker_runner(
            arms=arms, bundles=bundles, configuration=args.configuration,
            timeout_seconds=args.timeout_seconds,
        ),
    ...
    print(json.dumps({
        "checkpoint": str(args.checkpoint), "records": len(report["records"]),
        "complete": report["complete"], "execution_failures": failures,
    }, sort_keys=True))
