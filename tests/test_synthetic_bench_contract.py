...
class PublicationFirstClaimTests(unittest.TestCase):
    ...
    def test_tracked_reference_package_is_manuscript_ready(self):
        ...
        self.assertEqual(verdicts["manuscript_claim_coverage"],
                         {"complete": True, "reasons": []})
        self.assertEqual(verdicts["manuscript_publication_readiness"],
                         {"ready": True, "reasons": []})

    def test_declared_ratio_is_bound_to_the_artifact_row(self):
        ...
        claim["expected_baseline_over_router"] = [99.0] * len(
            claim["artifact_case_mapping"])

        verdicts = paper_claims.audit_candidate_package(ROOT, registry)

        self.assertIn(
            "claim_artifact_ratio_mismatch:grouped.dgelsy:"
